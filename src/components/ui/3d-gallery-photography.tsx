...
// Direct module import: drei's barrel also drags in hls.js (~570 KB) and
// MediaPipe (~130 KB) as lazy chunks the gallery never uses.
import { useTexture } from '@react-three/drei/core/Texture';
import * as THREE from 'three';

// Infinite 3D photo gallery (react-three-fiber).
//
// Adapted for this repo (React 18 → @react-three/fiber 8 + drei 9):
//  · `speed` and `visibleCount` now reach the scene (the original accepted
//    them but never passed them down, so they had no effect)
//  · the wheel listener binds to THIS gallery's canvas, not whichever
//    <canvas> happens to be first in the document
//  · textures load inside a <Suspense> boundary
//  · touch drag moves the gallery (the demo text promised it; the original
//    had no touch handling)
// `zSpacing` and `falloff` are accepted for API compatibility but unused,
// as in the original.
//
// Used on /shop through src/islands/shop-gallery.tsx (the shop is a static
// page, so it loads a pre-bundled island — see scripts/build-islands.cjs).
...
function GalleryScene({
	...
	const spatialPositions = useMemo(() => {
		...
		for (let i = 0; i < visibleCount; i++) {
			// Golden-angle distribution on both axes
	...
	const nudge = useCallback((amount: number) => {
		setScrollVelocity((prev) => prev + amount);
		setAutoPlay(false);
	...
	}, [gl, handleWheel, handleKeyDown, nudge, speed]);
	...
	useFrame((state, delta) => {
		...
		planesData.current.forEach((plane, i) => {
			...
			// Opacity from fade settings (0 → 1 → 0 along the depth range)
	...
	return (
		<>
			{planesData.current.map((plane, i) => {
				...
				// Keep each photo's aspect ratio
